 (ns malletfn.viz.dmrtopics-features-viz
  (:use rosado.processing)
  (:use malletfn.topicmodel)
  (:use malletfn.dmrtopics)
...
;; (def dmr (load-dmr "rhinoplastfm.ser" 1000 16))
    
;(def input-file "resources/rhinoplastfm.ser")
;(def num-iterations 1000)
;(def num-topics 16)

;(def input-file "resources/dmr-docs.ser")
;(def num-iterations 1000)
;(def num-topics 8)

(def num-topics (:topics dmr-params-map))
(def input-file (:corpus dmr-params-map))

(def dmr (load-dmr dmr-params-map))
(def topic-assignments (.getData dmr))
                 ...
                 (binding [*applet* this]
                   (let [num-drawn (draw-docs this 
                                              ...
                                              (line-xy (- (width) 10) (- (height) 20) 
                                                       (- (width) 10) (viz-const :y-line-height)))]
                     (swap! page-size (fn [x] num-drawn))
                     (swap! drawn (fn [x] true))))))
       (keyPressed [evt]
             (binding [*applet* this]
               (swap! start-index (fn [si] (+ si @page-size)))))
       (mouseClicked [evt]
             (binding [*applet* this]
               (swap! drawn (fn [x] false))))))
